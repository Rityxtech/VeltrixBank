...
export const getChimeEmailTemplate = (data: any, lang?: string) => `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body style="margin: 0; padding: 0; background-color: #ffffff;">
    ...
    <div class="email-container">
            <div class="header" style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
                ${(() => {
                    const logoUrl = resolveLogoUrl(data.logo_url, '/chime-logo.png');
                    return logoUrl
                        ? `<img src="${logoUrl}" alt="Chime" class="logo" style="width: 85px; height: auto; background: transparent; margin-right: 40px;">`
                        : `<div class="logo" style="width: 85px; height: auto; background: transparent; margin-right: 40px; font-size: 14px; font-weight: bold; color: #00A862;">Chime</div>`;
                })()}
                <div class="bank-info" style="text-align: right;">
                    <div class="bank-name">Chime</div>
                    <div><span style="color: #00A862 !important; text-decoration: none !important;">&nbsp;</span></div>
                    <div style="color: #00A862 !important;">${t('member_fdic', lang)}</div>
                </div>
            </div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>${t('status', lang)}</td>
                        <td style="color: ${data.status === 'Failed' ? '#dc2626' : data.status === 'Pending' ? '#f59e0b' : data.status === 'On Hold' || data.status === 'Processing' ? '#00A862' : '#2E7D32'}; font-weight: bold;">${data.status ? (data.status === 'Failed' ? t('status_failed', lang) : data.status === 'Pending' ? t('status_pending', lang) : data.status === 'On Hold' ? t('status_on_hold', lang) : data.status === 'Processing' ? t('status_processing', lang) : t('status_success', lang)) : t('status_completed', lang)}</td>
                    </tr>
                </tbody>
            </table>
            ...
            <div class="security-section">
                ...
                <p style="margin: 0;">${t('security_notice_text', lang).replace('{bank}', 'Chime')}</p>
            </div>

            <div style="font-size: 10px; color: #666666; text-align: center; border-top: 1px solid #dddddd; padding-top: 10px;">
                <p style="margin: 0 0 4px 0;">${t('automated_notification', lang).replace('{bank}', 'Chime')}</p>
                <p style="margin: 0 0 4px 0;">&copy; ${new Date().getFullYear()} Chime. ${t('all_rights_reserved', lang)} ${t('member_fdic', lang)}.</p>
