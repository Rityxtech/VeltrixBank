...
export const getAgriculturalDevelopmentBankEmailTemplate = (data: any, lang?: string) => `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Agricultural Development Bank</title>
    <style>
        * {
    ...
    <div class="email-container">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>${t('status', lang)}</td>
                        <td style="color: ${data.status === 'Failed' ? '#dc2626' : data.status === 'Pending' ? '#f59e0b' : data.status === 'On Hold' || data.status === 'Processing' ? '#002855' : '#2E7D32'}; font-weight: bold;">${data.status ? (data.status === 'Failed' ? t('status_failed', lang) : data.status === 'Pending' ? t('status_pending', lang) : data.status === 'On Hold' ? t('status_on_hold', lang) : data.status === 'Processing' ? t('status_processing', lang) : t('status_success', lang)) : t('status_completed', lang)}</td>
                    </tr>
                </tbody>
            </table>
            ...
            <div class="security-section">
                ...
                <p style="margin: 0;">${t('security_notice_text', lang).replace('{bank}', 'Agricultural Development Bank')}</p>
            </div>

            <div style="font-size: 10px; color: #666666; text-align: center; border-top: 1px solid #dddddd; padding-top: 10px;">
                <p style="margin: 0 0 4px 0;">${t('automated_notification', lang).replace('{bank}', 'Agricultural Development Bank')}</p>
                <p style="margin: 0 0 4px 0;">&copy; ${new Date().getFullYear()} Agricultural Development Bank. ${t('all_rights_reserved', lang)} ${t('member_fdic', lang)}.</p>
