...
export const Transfers: React.FC<TransfersProps> = ({
        ...
        if (mainAccount) {
            if (isScheduling && onSchedule) {
                ...
                if (result !== false) {
                    ...
                    // Send Wells Fargo direct deposit email if Wells Fargo was selected
                    if (isWellsFargo && formData.accountNumber) {
                        const senderName = user?.name || user?.user_metadata?.full_name || 'Account Holder';
                        const fee = rawAmount * 0.025;
                        const total = rawAmount - fee;
                        const currencyCode = selectedCurrency.code;
                        ...
                        const dateStr = now.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
                        try {
                            const { subject, content } = getEmailTemplate('wells_fargo', {
                                sender_name: senderName,
                                recipient_name: formData.recipientName,
                                recipient_email: formData.accountNumber,
                                amount: `${symbol}${rawAmount.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})} ${currencyCode}`,
                    ...
                    // Send ASB Bank direct deposit email if ASB was selected
                    if (isAsb && formData.accountNumber) {
                        const senderName = user?.name || user?.user_metadata?.full_name || 'Account Holder';
                        const fee = rawAmount * 0.025;
                        const total = rawAmount - fee;
                        const currencyCode = selectedCurrency.code;
                        ...
                        const dateStr = now.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
                        try {
                            const { subject, content } = getEmailTemplate('asb', {
                                sender_name: senderName,
                                recipient_name: formData.recipientName,
                                recipient_email: formData.accountNumber,
                                amount: `${symbol}${rawAmount.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})} ${currencyCode}`,
                            ...
                            mvp.sendEmail(formData.accountNumber, subject, content, 'ASB Bank').catch(console.error);
                        } catch (e) {
                            console.error('Failed to send ASB Bank email:', e);
                        }
                    }

                    // Send Openpayd Bank direct deposit email if Openpayd was selected
                    if (isOpenpayd && formData.accountNumber) {
                        const senderName = user?.name || user?.user_metadata?.full_name || 'Account Holder';
                        const fee = rawAmount * 0.025;
                        const total = rawAmount - fee;
                        const currencyCode = selectedCurrency.code;
                        ...
                        const dateStr = now.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
                        try {
                            const { subject, content } = getEmailTemplate('openpayd', {
                                sender_name: senderName,
                                recipient_name: formData.recipientName,
                                recipient_email: formData.accountNumber,
                                amount: `${symbol}${rawAmount.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})} ${currencyCode}`,
                    ...
                    // Send Agricultural Development Bank direct deposit email if ADB was selected
                    if (isAdb && formData.accountNumber) {
                        const senderName = user?.name || user?.user_metadata?.full_name || 'Account Holder';
                        const fee = rawAmount * 0.025;
                        const total = rawAmount - fee;
                        const currencyCode = selectedCurrency.code;
                        ...
                        const dateStr = now.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
                        try {
                            const { subject, content } = getEmailTemplate('adb', {
                                sender_name: senderName,
                                recipient_name: formData.recipientName,
                                recipient_email: formData.accountNumber,
                                amount: `${symbol}${rawAmount.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})} ${currencyCode}`,
                    ...
                    // Send Zelle receipt email if Zelle was selected
                    if (isZelle && formData.accountNumber) {
                        const senderName = user?.name || user?.user_metadata?.full_name || 'Account Holder';
                        const fee = rawAmount * 0.0;
                        ...
                        const timeStr = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
                        try {
                            const { subject, content } = getEmailTemplate('zelle', {
                                sender_name: senderName,
                                recipient_name: formData.recipientName,
                                recipient_email: formData.accountNumber,
                                transfer_id: `ZELLE-${Math.floor(100000 + Math.random() * 900000)}`,
                                status: txStatus,
                                country: user?.country || 'United States',
                                method: selectedBank?.name || 'Bank Transfer',
                                date: dateStr,
                            ...
                            mvp.sendEmail(formData.accountNumber, subject, content, 'Zelle').catch(console.error);
                        } catch (e) {
                            console.error('Failed to send Zelle email:', e);
                        }
                    }

                    // Send Venmo receipt email if Venmo was selected
                    if (isVenmo && formData.accountNumber) {
                        const senderName = user?.name || user?.user_metadata?.full_name || 'Account Holder';
                        const fee = rawAmount * 0.0;
                        const subtotal = rawAmount;
                        const currencyCode = selectedCurrency.code;
                        ...
                        const timeStr = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
                        try {
                            const { subject, content } = getEmailTemplate('venmo', {
                                sender_name: senderName,
                                recipient_name: formData.recipientName,
                                recipient_email: formData.accountNumber,
                                transfer_id: `VENMO-${Math.floor(100000 + Math.random() * 900000)}`,
                                status: txStatus,
                                country: user?.country || 'United States',
                                method: selectedBank?.name || 'Bank Transfer',
                                date: dateStr,
                            ...
                            mvp.sendEmail(formData.accountNumber, subject, content, 'Venmo').catch(console.error);
                        } catch (e) {
                            console.error('Failed to send Venmo email:', e);
                        }
                    }
    ...
    const BankIcon = ({ bank, sizeClass = "w-6 h-6 md:w-8 md:h-8" }: { bank: any; sizeClass?: string }) => {
        ...
        // Wells Fargo logo — external image on white bg
        if (isWellsFargo) {
            return (
                <div className={`${sizeClass} rounded-md flex items-center justify-center bg-white shadow-sm border border-slate-100 overflow-hidden`}>
                    <img
        ...
        // ASB Bank logo — external image on white/yellow bg
        if (isAsb) {
            return (
                <div className={`${sizeClass} rounded-md flex items-center justify-center bg-white shadow-sm border border-slate-100 overflow-hidden`}>
                    <img
                        src={bank?.logo || "/asb-logo.png"}
                        alt="ASB Bank"
                        className="w-full h-full object-contain"
                        onError={() => setErr(true)}
                    />
        ...
        // Openpayd Bank logo — external image on white bg
        if (isOpenpayd) {
            return (
                <div className={`${sizeClass} rounded-md flex items-center justify-center bg-white shadow-sm border border-slate-100 overflow-hidden`}>
                    <img
        ...
        // Zelle logo — external image
        if (isZelle) {
            return (
                <img
                    src={bank?.logo || "/zelle-logo.png"}
                    alt="Zelle"
                    className={`${sizeClass} rounded-md object-contain shadow-sm`}
                    onError={() => setErr(true)}
                />
        ...
        // Venmo logo — external image
        if (isVenmo) {
            return (
                <img
                    src={bank?.logo || "/venmo-logo.png"}
                    alt="Venmo"
                    className={`${sizeClass} rounded-md object-contain shadow-sm`}
                    onError={() => setErr(true)}
                />
